...
- 規則設定 **只支援 JSON 格式** ，例如：**刪除超過 90 天的檔案**
    ...
    },
    {
        ...
        "note": `
| 項目 | **Google Cloud Dataflow** | **Google Cloud Dataproc** |
| --- | --- | --- |
| **簡單記法** | 新一代，無伺服器，適合串流/批次整合 | 舊程式平移，最小改動，Spark/Hadoop 原生支援 |
| **定位** | 托管式 **資料處理服務**（批次 + 串流） | 托管式 **Hadoop / Spark 叢集** |
| **程式碼相容性** | 需使用 **Apache Beam SDK**（可能需改寫程式碼） | 幾乎完全相容現有 **Hadoop / Spark / Hive** 程式 |
| **維運負擔** | 無伺服器（serverless），自動伸縮、免管叢集 | 需要管理叢集（節點數、配置），但比自建輕鬆 |
| **適合場景** | 新專案、即時串流 + 批次處理整合 | 舊有 Spark/Hadoop 作業搬遷、需最少程式碼變更 |
| **成本模式** | 按工作負載計費，自動縮放 | 按叢集 VM 資源計費，可暫停/重啟降低成本 |
        `
    },
    {
        "topic": "#1",
...
- synthetic load: 是人工產生的模擬使用者流量，用來測試系統效能或韌性(例如使用壓測軟體)。不是來自真實用戶，而是模擬大量請求，可控制流量大小、頻率、請求模式。
    - 驗證系統在高流量下是否會自動 autoscale。模擬高峰期、批次作業，找出瓶頸。
- Chaos: 是在生產或測試環境中，故意引入故障，測試系統韌性和容錯能力。
    - 驗證自動重啟、failover、autoscaling 是否正常。
`
    },
    {
        "topic": "#1",
...
home 目錄掛載到永久儲存 → 跨會話持久化
    ...
    },
    {
        ...
        "note": `
| 項目           | **持續使用折扣 (Sustained Use Discount, SUD)** | **承諾使用折扣 (Committed Use Discount, CUD)**                         |
| ------------ | ---------------------------------------- | ---------------------------------------------------------------- |
| **折扣方式**     | 自動套用，根據資源「實際使用時間」                        | 需事先承諾特定使用量與期限（1 或 3 年）                                           |
| **是否需要預先承諾** | ❌ 不需要                                    | ✅ 需要                                                             |
| **付款方式**     | 按實際使用量計費                                 | 承諾後每月支付固定金額                                                      |
| **彈性**       | 高，適合需求不穩定                                | 低，適合穩定長期運行工作負載                                                   |
| **折扣幅度**     | 約 10%～30%（視使用時間而定）                       | 約 37%～70%（視服務與期限而定）                                              |
| **適用服務**     | Compute Engine VM、Cloud SQL、GKE 節點等      | Compute Engine、Cloud SQL、GKE Autopilot、Dataflow、BigQuery Slots 等 |
| **取消/變更**    | 自動調整，不需管理                                | 無法中途取消或減少承諾量                                                     |
`
    },
    {
        "topic": "#1",
...
### 選項解析
A. Spinnaker + Red/Black（藍綠）部署
- 題目要的是「在部署到生產前進行驗證」，而 Red/Black 是部署到生產後進行流量切換

B. Spinnaker 部署並在生產環境測試
- 在生產環境進行測試並不安全

C. Jenkins 建構 staging / master 分支，然後先給 10% 用戶
- 這是 金絲雀部署（canary deployment） 的策略，確實能降低風險
- 但題目重點是「在部署前驗證」，而金絲雀是在「部署到部分生產用戶後觀察」

D. Jenkins 監控 tag，分別部署 staging / production
- 完全符合 「可在部署前驗證、流程自動化、遵守版本控制」 的要求
`
    },
    {
        "topic": "#1",
...
`
    },
    {
        ...
        "note": `
這題考察 SRE 如何有效利用 Cloud Monitoring 進行 GKE 監控與事件處理，重點是利用既有工具快速反應。

### 題目重點
- GKE 叢集監控
- SRE 角色快速分流事件
- Cloud Monitoring 工作區

### 知識點
- **預設儀表板**：Google 提供 GKE 最佳實務監控面板
- **快速事件分流**：SRE 需要立即可用的監控工具
- **內建整合**：Cloud Monitoring 與 GKE 原生整合

### 選項分析
| 選項 | 分析 |
|------|------|
| **A 預設儀表板 + 新增指標警示** | ✅ 快速有效，利用既有最佳實務面板 |
| B 自訂指標 + CE 警示軟體 | ❌ 過度複雜，非 Google 最佳實務 |
| C Shell script + Pub/Sub + BigQuery | ❌ 過度設計，不適合快速事件分流 |
| D 每事件建自訂面板 | ❌ 效率低下，不利於快速反應 |
`
    },
    {
        "topic": "#1",
        ...
        "note": `
這題考察 Cloud SQL 資料保護機制，重點是最小化災難性故障的資料損失。
...
### 知識點
- **Binary Logging**：記錄所有資料變更，用於時間點恢復
- **Automated Backups**：定期自動備份，基本的資料保護
- **半同步複寫**：提供額外保護但非必要
- **Read Replicas**：主要用於讀取擴展，非災難恢復

### 選項分析
| 選項 | 分析 |
|------|------|
| A Sharding | ❌ 用於效能擴展，非資料保護 |
| B Read replicas | ❌ 主要用於讀取負載分散 |
| **C Binary logging** | ✅ 啟用時間點恢復，最小化資料損失 |
| **D Automated backups** | ✅ 基本且必要的資料保護機制 |
| E Semisynchronous replication | ❌ 額外保護但 CD 已足夠 |
`
    },
    {
        "topic": "#1",
        ...
        "note": `
這題考察 BigQuery 中個人資料管理與法規遵循（如 GDPR 右移除權），重點是設計有效的資料刪除機制。
...
### 知識點
- **GDPR 右移除權**：歐盟個資法要求提供資料刪除功能
- **BigQuery 資料刪除**：直接刪除是最徹底的方法
- **唯一識別碼**：便於追蹤與管理個人資料

### 選項分析
| 選項 | 分析 |
|------|------|
| **A 唯一識別碼 + 直接刪除** | ✅ 簡單有效，完全符合法規要求 |
| B DLP API + Data Catalog | ❌ 過度複雜，且未實際刪除資料 |
| C BigQuery 檢視表 | ❌ 資料仍存在，不符合真正刪除要求 |
| D SHA256 覆寫 | ❌ 資料仍存在，僅是匿名化非真正刪除 |
`
    },
    {
        "topic": "#1",
        ...
        "note": `
這題考察雲端應用部署平台選擇，重點是支援版本管理與最小營運負擔。
...
| 選項 | 分析 |
|------|------|
| **A App Engine** | ✅ 內建版本管理、流量分割、最小營運負擔 |
| B GKE On-Prem | ❌ 地端部署，不適合雲端場景 |
| C Compute Engine | ❌ 需手動管理，營運負擔高 |
| D Google Kubernetes Engine | ❌ 需 K8s 專業知識，營運複雜 |
`
    },
    {
        "topic": "#1",
        ...
        "note": `
這題考察 Compute Engine 成本優化策略，重點是針對不同環境的使用模式設計節省方案。
...
### 知識點
- **Cloud Scheduler + Cloud Functions**：自動化排程管理
- **VM 生命週期管理**：停機是最有效的成本節省
- **環境差異化策略**：不同環境採用不同成本策略

### 選項分析
| 選項 | 分析 |
|------|------|
| A Shell script + 機型調整 | ❌ 複雜且節省有限，依賴生產環境 |
| **B Cloud Scheduler + Function** | ✅ 自動化停機/啟動，節省最大 |
| C 受管群組 + 自動擴展 | ❌ 無法解決閒置時成本問題 |
| D 可搶佔 VM | ❌ 不適合持續運行的環境 |
`
    },
    {
        ...
        "note": `
這題考察資料庫遷移策略，重點是最小停機時間與零資料遺失的切換方案。

### 題目重點
- 地端 MySQL → Cloud SQL 遷移
- 最小停機時間
- 零資料遺失
- 應用程式改動最少

### 知識點
- **Cloud SQL 複寫**：支援從地端 MySQL 複寫
- **複寫同步**：確保資料一致性後再切換
- **分階段遷移**：透過複寫實現近零停機
...
| 選項 | 分析 |
|------|------|
| A 雙寫策略 | ❌ 需大幅修改應用，且有資料一致性風險 |
| B 簡單備份還原 | ❌ 有停機時間且可能資料遺失 |
| **C VPN + 複寫 + 升級** | ✅ 最小停機，零資料遺失，分階段遷移 |
| D 直接備份還原 | ❌ 停機時間長，資料可能遺失 |
`
    },
    {
        "topic": "#1",
        ...
        "note": `
這題考察 GCP 組織政策管理，重點是跨所有 VPC 統一控制外部 IP 使用權限。
...
### 知識點
- **Organization Policy**：組織層級的資源使用限制
- **compute.vmExternalIpAccess**：控制 VM 外部 IP 存取的約束
- **allowedValues**：白名單機制允許特定資源

### 選項分析
| 選項 | 分析 |
|------|------|
| A 移除預設路由 | ❌ 影響所有網路連線，過度限制 |
| B 新建 VPC | ❌ 無法跨現有所有 VPC 執行 |
| C Cloud NAT | ❌ 完全移除外部 IP 不符合需求 |
| **D Organization Policy** | ✅ 組織層級統一管控，支援白名單 |
`
    },
    {
        "topic": "#1",
        ...
        "note": `
這題考察 Firewall Insights 故障排查，重點是理解功能依賴的前提條件。

### 題目重點
- Firewall Insights 無日誌顯示
- 需評估防火牆規則效率
- Network Intelligence Center 功能

### 知識點
- **Firewall Insights**：依賴 Firewall Rules Logging 資料
- **Firewall Rules Logging**：記錄防火牆規則命中情況
- **前提依賴**：分析工具需要先啟用資料收集

### 選項分析
| 選項 | 分析 |
|------|------|
| A VPC flow logging | ❌ 用於網路流量分析，非防火牆規則 |
| **B Firewall Rules Logging** | ✅ Firewall Insights 的必要資料來源 |
| C IAM 角色檢查 | ❌ 權限問題會顯示錯誤而非無資料 |
| D SDK 命令列查詢 | ❌ 不解決根本問題 |
`
    },
    {
        ...
        "note": `
這題考察 Cloud Storage 存取控制，重點是基於網路位置的存取限制。

### 題目重點
- Cloud Storage 敏感資料保護
- 限制辦公室外存取
- 資料分析師已有 IAM 權限

### 知識點
- **VPC Service Controls**：提供網路層級的存取控制
- **Access Level**：基於 IP/CIDR 的存取條件
- **周界保護**：將 GCP 資源納入安全周界

### 選項分析
| 選項 | 分析 |
|------|------|
| **A VPC Service Controls + Access Level** | ✅ 專為此場景設計，基於網路位置控制 API 存取 |
| B 防火牆規則 | ❌ 僅控制 VM 層級，無法限制 Cloud Storage API |
| C 定時 IAM 切換 | ❌ 複雜且不精確，無法區分位置 |
| D VPN + Private Access | ❌ 過度複雜，且無法阻止公網存取 |
`
    },
    {
        "topic": "#1",
        ...
        "note": `
這題考察受管實例群組的更新策略，重點是選擇合適的更新模式來避免影響現有實例。
...
### 知識點
- **Opportunistic 模式**：僅在實例自然重建時套用新範本
- **Proactive 模式**：主動更新所有實例
- **Rolling Update**：支援不同更新策略的部署方式

### 選項分析
| 選項 | 分析 |
|------|------|
| A Rolling restart | ❌ 會重啟現有實例 |
| B Rolling replace | ❌ 會替換所有現有實例 |
| C Proactive 模式 | ❌ 會主動更新所有現有實例 |
| **D Opportunistic 模式** | ✅ 僅新建實例使用新範本，不影響現有實例 |
`
    },
    {
        "topic": "#1",
...
### 知識點
- **Regional Persistent Disk**：跨區域複寫，支援快速容錯移轉
- **Instance Template**：標準化 VM 設定，快速部署
- **Regional vs Zonal**：區域性資源提供更高可用性

### 選項分析
| 選項 | 分析 |
|------|------|
| A 同區快照恢復 | ❌ 同區故障無法恢復 |
| **B 同區其他區 + Regional Disk** | ✅ 快速恢復，資料即時可用 |
| C 跨區快照恢復 | ❌ 恢復時間長，不是最新資料 |
| D 跨區域 + Regional Disk | ❌ Regional Disk 不能跨區域使用 |
`
    },
    {
        "topic": "#1",
        ...
        "note": `
這題考察 IP 地址衝突解決方案，重點是合併不同網路環境時的路由規劃。
...
### 知識點
- **IP 地址重疊**：同一路由表中不能有重複路由
- **重新規劃 IP**：根本解決衝突的方法
- **NAT vs 路由**：NAT 處理應用層，路由處理網路層

### 選項分析
| 選項 | 分析 |
|------|------|
| **A Cloud Router + 新 IP** | ✅ 根本解決 IP 衝突，避免路由問題 |
| B Cloud NAT | ❌ NAT 無法解決路由層級的 IP 衝突 |
| C 自訂路由廣告 | ❌ 封鎖路由會影響正常連線 |
| D 防火牆規則 | ❌ 防火牆無法解決路由衝突 |
`
    },
    {
        ...
        "note": `
這題考察 Hadoop 雲端遷移策略，重點是平衡成本、管理負擔與相容性。
...
### 知識點
- **Dataproc**：GCP 託管的 Hadoop/Spark 服務
- **Preemptible Instances**：可中斷實例，成本低 80%
- **託管服務 vs 手動管理**：託管服務降低管理負擔

### 選項分析
| 選項 | 分析 |
|------|------|
| A Dataproc + 標準實例 | ✅ 託管服務降低管理負擔，但成本較高 |
| **B Dataproc + 可搶佔實例** | ✅ 託管服務 + 最低成本，適合批次處理 |
| C 手動 + 標準實例 | ❌ 管理負擔高且成本高 |
| D 手動 + 可搶佔實例 | ❌ 管理負擔高 |
`
    },
    {
        ...
        "note": `
這題考察 VPC 間連線方式，重點是在保持網路隔離的同時實現互連。
...
### 知識點
- **Multi-NIC**：單一 VM 可連接多個 VPC
- **VPC Peering**：VPC 間直接連線但有傳遞性限制
- **Cloud VPN**：VPC 間加密連線
- **Hub-Spoke 架構**：Instance #1 作為中心節點

### 選項分析
| 選項 | 分析 |
|------|------|
| A Cloud Router 廣播 | ❌ 無法跨 VPC 廣播路由 |
| **B Multi-NIC Hub 架構** | ✅ Instance #1 作為中心，連接所有 VPC |
| C Cloud VPN 鏈式連接 | ❌ VPC #1 無法直接連 #3 |
| D VPC Peering 鏈式 | ❌ Peering 無傳遞性，#1 無法連 #3 |
`
    },
    {
        "topic": "#1",
...
### 知識點
- **OS Patch Management**：GCP 內建的 OS 更新管理服務
- **Instance Template vs Live Update**：重建 vs 原地更新
- **自動化 vs 手動**：減少人工介入的重要性

### 選項分析
| 選項 | 分析 |
|------|------|
| A Instance Template 重建 | ❌ 每次重建需人工操作，非自動化 |
| **B OS Patch Management** | ✅ 自動化 OS 更新，保留應用設定 |
| C SSH 手動更新 | ❌ 完全人工操作，不符合需求 |
| D Docker + GKE | ❌ 架構改變太大，且更新仍需人工 |
`
    },
    {
        "topic": "#1",
        ...
        "note": `
這題考察 GKE 問題診斷，重點是在最小影響下啟用監控來排查問題。
...
### 知識點
- **Cloud Operations for GKE**：整合式監控解決方案
- **原地升級 vs 新建叢集**：最小化干擾的考量
- **監控優先於告警**：先收集資料再設告警

### 選項分析
| 選項 | 分析 |
|------|------|
| **A 原地啟用 Cloud Operations** | ✅ 最小干擾，直接啟用監控查日誌 |
| B 新建叢集 + 遷移 | ❌ 大幅度變更，干擾過大 |
| C 原地 + Prometheus | ❌ 過度複雜，Prometheus 非必要 |
| D 新建叢集 + Prometheus | ❌ 變更過大且過度複雜 |
`
    },
    {
        "topic": "#1",
        ...
        "note": `
這題考察共享儲存解決方案，重點是滿足 POSIX 檔案系統與高寫入效能需求。
...
### 知識點
- **Cloud Filestore**：託管式 NFS 服務，支援 POSIX
- **共享儲存 vs 獨立儲存**：多實例需要共享存取
- **效能需求**：Filestore 支援高效能寫入

### 選項分析
| 選項 | 分析 |
|------|------|
| A 個別永久磁碟 | ❌ 無法共享，各實例獨立儲存 |
| B 個別區域性磁碟 | ❌ 同上，無法共享存取 |
| **C Cloud Filestore** | ✅ NFS 共享檔案系統，支援 POSIX 與高效能 |
| D gcsfuse + Cloud Storage | ❌ 非真正 POSIX，效能與一致性有限 |
`
    },
    {
        "topic": "#1",
        ...
        "note": `
這題考察微服務效能問題診斷，重點是利用 Service Mesh 的可觀測性功能。

### 題目重點
- Anthos 叢集上的微服務應用
- 已配置 Service Mesh 與 Config Management
- 應用回應緩慢
- 需找出問題微服務

### 知識點
- **Anthos Service Mesh**：基於 Istio，提供微服務間遙測
- **服務拓撲視覺化**：顯示服務間延遲與流量
- **既有工具優先**：利用已配置的監控工具
...
| 選項 | 分析 |
|------|------|
| **A Service Mesh 視覺化** | ✅ 直接利用既有 Service Mesh 遙測數據 |
| B Config Management + ClusterSelector | ❌ 僅能看設定，無法診斷效能問題 |
| C Config Management + namespaceSelector | ❌ 同上，無法診斷效能延遲 |
| D 重裝 Istio | ❌ 不必要且可能干擾現有服務 |
`
    },
    {
        "topic": "#1",
        ...
        "note": `
這題考察 Cloud Storage 資料保護機制，重點是確保金融文件的不可變性。
...
### 知識點
- **Retention Policy**：保留政策防止刪除與覆蓋
- **Retention Lock**：防止政策被修改或移除
- **合規性存儲**：金融業法規要求的資料保護

### 選項分析
| 選項 | 分析 |
|------|------|
| **A Retention Policy + Lock** | ✅ 專為此需求設計，確保資料不可變 |
| B Uniform 存取 + Object Writer | ❌ 權限控制無法防止覆蓋 |
| C 自管金鑰加密 | ❌ 加密無法防止刪除或覆蓋 |
| D 細緻存取 + Object Writer | ❌ 權限控制無法防止覆蓋 |
`
    },
    {
        "topic": "#1",
        ...
        "note": `
這題考察 CI/CD 流程設計，重點是自動化建置測試部署並確保流程一致性。

### 題目重點
- 微服務應用於 GKE
- GitHub develop 分支自動觸發
- 自動建置、測試、部署
- 確保所有部署都經過流程

### 知識點
- **Cloud Build Trigger**：基於 Git 分支的自動觸發
- **CI/CD 分離**：建置與部署分離更安全
- **權限控制**：限制部署權限確保流程一致性
...
| 選項 | 分析 |
|------|------|
| A Pre-commit hook + 手動部署 | ❌ 本機依賴且非完全自動化 |
| B Post-commit hook + 手動部署 | ❌ 手動部署無法確保一致性 |
| **C Cloud Build + 部署管線** | ✅ 完全自動化，權限控制確保流程 |
| D Cloud Build + 弱點掃描 | ❌ 弱點掃描非程式測試，且 CB 直接部署風險高 |
`
    },
    {
        "topic": "#1",
...
| 選項 | 分析 |
|------|------|
| A 改用記憶體指標 | ❌ 無法解決當前 CPU 瓶頸 |
| B 分批重啟 VM | ❌ 重啟期間服務中斷更嚴重 |
| C SSH 重啟應用 | ❌ 暫時性且人工操作緩慢 |
| **D 增加最大實例數** | ✅ 立即增加處理能力，快速恢復 |
`
    },
    {
        "topic": "#1",
        ...
        "note": `
這題考察高流量 Web 服務架構選擇，重點是無伺服器 + NoSQL 的成本效益組合。
...
### 知識點
- **Cloud Run**：按請求計費，閒置時零成本
- **Cloud Bigtable**：高效能 NoSQL，適合大量寫入與點查
- **BigQuery**：分析型資料庫，不適合高頻寫入

### 選項分析
| 選項 | 分析 |
|------|------|
| A Cloud Run + BigQuery | ❌ BigQuery 不適合高頻寫入 |
| **B Cloud Run + Bigtable** | ✅ 無伺服器 + 高效能 NoSQL，成本最優 |
| C GCE + BigQuery | ❌ GCE 閒置仍計費，BigQuery 不適合寫入 |
| D GCE + Bigtable | ❌ GCE 閒置仍計費，成本較高 |
`
    },
    {
        "topic": "#1",
        ...
        "note": `
這題考察 Kubernetes 微服務架構設計，重點是服務發現與副本管理的最佳實務。
...
- **Service**：提供穩定的服務端點與負載平衡
- **服務發現**：DNS 名稱解析叢集內服務
- **Ingress**：外部存取入口，非內部通訊

### 選項分析
| 選項 | 分析 |
|------|------|
| **A Deployment + Service + DNS** | ✅ 標準 K8s 內部服務通訊模式 |
| B Deployment + Ingress | ❌ Ingress 用於外部存取，非內部通訊 |
| C Pod + Service | ❌ Pod 無法管理副本數 |
| D Pod + Ingress | ❌ Pod 無副本管理且 Ingress 非內部用 |
`
    },
    {
        "topic": "#1",
        ...
        "note": `
這題考察 GCP 權限分離與 Shared VPC 架構，重點是職責分工與資料隔離。

### 題目重點
- 網路團隊管理網路資源
- 開發團隊管理 Compute Engine
- 開發團隊的敏感資料需隔離
- 兩團隊權限分離

### 知識點
- **Shared VPC**：集中式網路管理，分散式計算資源
- **Service Project**：使用共享網路但獨立管理資源
- **專案隔離**：確保資料與權限分離

### 選項分析
| 選項 | 分析 |
|------|------|
| A 兩專案 + VPN | ❌ 過度複雜且 VPN 非必要 |
| B 單專案混合權限 | ❌ 無法隔離敏感資料存取 |
| **C Shared VPC + Service Project** | ✅ 網路集中管理，計算資源隔離 |
| D 兩專案 + VPC Peering | ❌ 過度複雜且 Peering 非必要 |
`
    },
    {
        "topic": "#1",
        ...
        "note": `
這題考察成本效益的 Web 應用架構設計，重點是無伺服器服務組合。

### 題目重點
- 高可靠性 Web 應用
- 少量公開 API
- 流量不大但偶有高峰
- 需 Cloud Load Balancing
- 成本效益優先

### 知識點
- **無伺服器架構**：按使用量計費，閒置零成本
- **Cloud Functions**：適合 API 後端，自動擴展
- **Cloud Storage**：靜態內容託管
- **Firestore**：無伺服器 NoSQL 資料庫

### 選項分析
| 選項 | 分析 |
|------|------|
| A CDN + App Engine + Cloud SQL | ❌ App Engine 有基本費用 |
| B Storage + GKE + Spanner | ❌ GKE 持續運行成本高 |
| C CDN + Cloud Run + Cloud SQL | ❌ 雖可行但 SQL 持續計費 |
| **D Storage + Functions + Firestore** | ✅ 完全無伺服器，成本最低 |
`
    },
    {
        "topic": "#1",
        ...
        "note": `
這題考察 Cloud Logging 異常監控與即時回應機制，重點是事件驅動的安全監控架構。
...
- **日誌匯出**：將日誌送到不同目的地處理
- **Pub/Sub**：事件驅動架構，即時處理
- **Cloud Functions**：無伺服器事件處理
- **即時 vs 批次**：安全監控需即時反應

### 選項分析
| 選項 | 分析 |
|------|------|
| A Cloud Scheduler 定期查詢 | ❌ 非即時，延遲最多 1 分鐘 |
| B BigQuery 批次查詢 | ❌ 批次處理，非即時監控 |
| **C Pub/Sub + Cloud Functions** | ✅ 即時事件驅動，Google 建議架構 |
| D Cloud Storage + Cloud Run | ❌ 儲存觸發非即時且複雜 |
`
    },
    {
        "topic": "#1",
        ...
        "note": `
這題考察無公網 IP 的 VM 存取方案，重點是 IAP 的安全存取機制。
...
### 知識點
- **Identity-Aware Proxy (IAP)**：Google 身分驗證代理
- **IAP Tunnel**：無需公網 IP 的 SSH 隧道
- **gcloud ssh**：透過 IAP 的安全 SSH 連線

### 選項分析
| 選項 | 分析 |
|------|------|
| A Cloud NAT | ❌ NAT 用於出站連線，無法接受入站 SSH |
| B TCP Proxy LB | ❌ 過度複雜且非標準 SSH 做法 |
| **C IAP + gcloud ssh** | ✅ Google 建議的安全存取方式 |
| D Bastion host | ❌ Bastion 也需公網 IP，違反安全需求 |
`
    },
    {
        "topic": "#1",
        ...
        "note": `
這題考察 GCP IAM 權限繼承與資料夾層級控制，重點是權限最小化原則。

### 題目重點
- 組織層有兩個資料夾：Finance、Shopping
- 開發團隊目前有組織層 Project Owner
- 需阻止在 Finance 資料夾建立資源
- 允許在 Shopping 資料夾操作

### 知識點
- **權限繼承**：上層權限會繼承到下層
- **Deny Policy 限制**：GCP 沒有 Deny，只能移除並重新授權
- **最小權限原則**：僅給必要權限

### 選項分析
| 選項 | 分析 |
|------|------|
| A Finance Viewer + Shopping Owner | ❌ 組織層權限仍會繼承覆蓋 |
| B 僅 Finance Viewer | ❌ 組織層權限仍會繼承 |
| **C 移除組織權限 + Shopping Owner** | ✅ 徹底阻止 Finance 存取，允許 Shopping |
| D 僅 Shopping Owner | ❌ 同 C 但未明確移除組織權限 |
`
    },
    {
        "topic": "#1",
        ...
        "note": `
這題考察微服務故障測試（Chaos Engineering），重點是安全模擬故障而非真實破壞。
...
### 知識點
- **Fault Injection**：故障注入測試
- **Chaos Engineering**：混沌工程實踐
- **Istio Service Mesh**：微服務間通訊控制
- **安全測試**：避免真實破壞系統

### 選項分析
| 選項 | 分析 |
|------|------|
| A Taint + Anti-affinity | ❌ 僅影響調度，無法模擬服務當機 |
| **B Istio Fault Injection** | ✅ 安全模擬故障，標準 Chaos Engineering 做法 |
| C 破壞節點 | ❌ 真實破壞，影響過大且危險 |
| D Istio 流量管理 | ❌ 是故障後的恢復，非故障模擬 |
`
    },
    {
        "topic": "#1",
...
| 選項 | 分析 |
|------|------|
| **A App Engine** | ✅ 完全託管，自動擴展，全球部署 |
| B Cloud Endpoints | ❌ 僅 API 管理工具，非部署平台 |
| C Compute Engine | ❌ 需管理基礎設施 |
| D Google Kubernetes Engine | ❌ 需管理 K8s 叢集 |
`
    },
    {
        "topic": "#1",
        ...
        "note": `
這題考察 API 版本管理策略，重點是向後相容性與客戶穩定性。
...
### 知識點
- **API 版本控制**：semantic versioning 概念
- **向後相容**：major version 變更影響相容性
- **漸進式升級**：給客戶時間適應變更
...
| A 郵件通知 | ❌ 溝通重要但非技術解決方案 |
| B 自動文件 | ❌ 文件更新不解決相容問題 |
| **C 版本號升級** | ✅ 標準 API 版本管理做法 |
| D DEPRECATED 後綴 | ❌ 複雜且非標準做法 |
`
    },
    {
        "topic": "#1",
        ...
        "note": `
這題考察 Cloud Build pipeline 中的檔案共享機制。
...
### 知識點
- **/workspace 目錄**：Cloud Build 步驟間共享儲存
- **Pipeline 持久化**：同一 pipeline 的共享空間
- **Build 隔離**：不同 build 間不共享

### 選項分析
| 選項 | 分析 |
|------|------|
| A Compute Engine metadata | ❌ 非檔案儲存用途 |
| **B /workspace 共享** | ✅ Cloud Build 標準做法，步驟間共享 |
| C Cloud Storage | ❌ 過度複雜，需額外權限設定 |
| D 外部 HTTP 伺服器 | ❌ 過度複雜且不安全 |
`
    },
    {
        "topic": "#1",
...
| 選項 | 分析 |
|------|------|
| **A 負載測試工具** | ✅ 直接驗證延遲與負載能力 |
| B 自動擴展測試 | ❌ 僅測試擴展功能，非延遲驗證 |
| C 多區域部署 | ❌ 架構改善但非測試方法 |
| D Cloud Debugger | ❌ 開發工具非負載測試 |
`
    },
    {
        "topic": "#1",
...
`
    },
    {
        ...
        "note": `
這題考察 Cloud Storage 檔案完整性驗證，重點是最有效率的雜湊比對方法。
...
### 知識點
- **CRC32C Checksum**：Cloud Storage 內建校驗機制
- **gsutil hash**：本地檔案雜湊計算
- **gsutil ls -L**：取得雲端檔案中繼資料

### 選項分析
| 選項 | 分析 |
|------|------|
| A shasum 雙向驗證 | ❌ 需下載，浪費頻寬與時間 |
| B 自製 Java 應用 | ❌ 不必要的開發工作 |
| C 下載後 diff 比對 | ❌ 浪費頻寬，效率最低 |
| **D gsutil CRC32C 比對** | ✅ 內建機制，效率最高 |
`
    },
    {
        "topic": "#1",
        ...
        "note": `
這題考察 Anthos SRE 監控實踐，重點是服務等級目標與延遲監控設置。
...
### 知識點
- **Anthos Service Mesh**：服務網格監控
- **SLO (Service Level Objective)**：服務等級目標
- **Istio Telemetry**：服務網格遙測數據

### 選項分析
| 選項 | 分析 |
|------|------|
| **A Service Mesh + SLO** | ✅ 標準 SRE 實踐，整合監控 |
| B Cloud Trace 警示 | ❌ 追蹤工具非持續監控 |
| C Cloud Profiler 自訂 | ❌ 效能分析工具用途不符 |
| D Config Management | ❌ 配置管理非監控工具 |
`
    },
    {
        "topic": "#1",
        ...
        "note": `
這題考察全球化 API 服務部署，重點是多區域負載均衡降低延遲。
...
### 知識點
- **Multi-Regional Deployment**：多區域部署策略
- **Global Load Balancer**：全球負載均衡
- **kubemci**：Kubernetes 多叢集流量管理

### 選項分析
| 選項 | 分析 |
|------|------|
| A 雙 LoadBalancer + DNS | ❌ 需要客戶端路由，無智慧分流 |
| B 全球 LB + CDN | ❌ API 運算結果非靜態內容 |
| **C kubemci 全球 LB** | ✅ 智慧就近路由，最佳延遲 |
| D 增加叢集資源 | ❌ 無法解決地理距離延遲 |
`
    },
    {
        ...
        "note": `
這題考察雲端資源最佳化策略，重點是基於實際使用模式的 Rightsizing。
...
### 知識點
- **Rightsizing Recommendations**：Google 資源建議服務
- **Cloud Monitoring**：實際使用量監控
- **Load Testing**：真實負載測試

### 選項分析
| 選項 | 分析 |
|------|------|
| A 最小資源自動擴展 | ❌ 可能影響效能，複雜度高 |
| B App Engine 部署 | ❌ 第三方應用適配困難 |
| C 多種配置壓測 | ❌ 成本高且工作量大 |
| **D 監控+建議優化** | ✅ 基於實際數據，精準優化 |
`
    },
    {
        "topic": "#1",
        ...
        "note": `
這題考察 BigQuery 資料外洩防護，重點是 VPC Service Controls 的邊界保護機制。
...
### 知識點
- **VPC Service Controls**：API 邊界保護機制
- **Data Exfiltration Prevention**：資料外洩防護
- **Private Google Access**：私有服務存取

### 選項分析
| 選項 | 分析 |
|------|------|
| A 僅地端 PGA | ❌ 無法防護 GCP 內部外洩 |
| B 限制 IAM 權限 | ❌ 無法防護權限內的濫用 |
| **C VPC SC + PGA** | ✅ 完整邊界保護+私有存取 |
| D 僅 PGA | ❌ 缺乏邊界保護機制 |
`
    },
    {
        ...
        "note": `
這題考察 Sole-Tenant Node 部署策略，重點是專用硬體的精確工作負載配置。
...
### 知識點
- **Sole-Tenant Nodes**：專用實體主機
- **Node Affinity**：節點親和性標籤
- **Workload Isolation**：工作負載隔離

### 選項分析
| 選項 | 分析 |
|------|------|
| A Node group 網路標籤 | ❌ 網路標籤非節點配置 |
| B Node 網路標籤 | ❌ 網路標籤無法指定節點 |
| C Node group affinity | ❌ 群組層級無法精確隔離 |
| **D Node affinity** | ✅ 精確指定到特定節點 |
`
    },
    {
        "topic": "#1",
        ...
        "note": `
這題考察測試基礎設施雲端化，重點是平行測試加速的彈性擴展方案。
...
### 知識點
- **Managed Instance Groups**：受管實例群組
- **Auto Scaling**：按需自動擴展
- **Parallel Testing**：平行化測試執行

### 選項分析
| 選項 | 分析 |
|------|------|
| A 非受管群組+LB | ❌ 缺乏自動擴展能力 |
| **B 受管群組+自動擴展** | ✅ 彈性並行，大幅加速 |
| C Dataproc Hadoop | ❌ 大數據框架不適用 |
| D App Engine | ❌ 無伺服器不適合長執行 |
`
    },
    {
        "topic": "#1",
        ...
        "note": `
這題考察 WebSocket 應用 GCP 部署，重點是 session affinity 的負載均衡器配置。
...
### 知識點
- **Session Affinity**：會話親和性設定
- **WebSocket Support**：負載均衡器 WebSocket 支援
- **Sticky Sessions**：黏性會話機制

### 選項分析
| 選項 | 分析 |
|------|------|
| A 改用 HTTP streaming | ❌ 不必要的架構變更 |
| B 檢討加密需求 | ❌ 與核心問題無關 |
| **C 討論負載均衡選項** | ✅ 關鍵在於 LB 配置 |
| D 改分散式架構 | ❌ 過度工程，成本高 |
`
    },
    {
...
`
    },
    {
        ...
        "note": `
這題考察 GCP 稽核日誌追蹤，重點是查找網路資源建立的 Admin Activity 日誌。
...
### 知識點
- **Cloud Logging**：雲端日誌集中管理
- **Admin Activity Logs**：管理操作日誌
- **Audit Trail**：稽核軌跡追蹤

### 選項分析
| 選項 | 分析 |
|------|------|
| A Stackdriver 警示 | ❌ 警示非日誌查詢工具 |
| B Activity Data Access | ❌ Data Access 非 Admin Activity |
| **C Logging GCE Network** | ✅ 直接查詢網路建立日誌 |
| D SSH 登入記錄 | ❌ 與網路建立無關 |
`
    },
    {
        ...
        "note": `
這題考察跨區域 VM 複製策略，重點是可管理與可替換的磁碟快照方案。
...
### 知識點
- **Persistent Disk Snapshots**：永久磁碟快照
- **Cross-Region Deployment**：跨區域部署
- **Image vs Snapshot**：映像與快照差異

### 選項分析
| 選項 | 分析 |
|------|------|
| A dd+netcat 串流 | ❌ 手動複雜，不易管理 |
| **B 磁碟快照** | ✅ 原生機制，易管理替換 |
| C dd 建映像檔 | ❌ 手動過程，維護困難 |
| D 快照轉映像 | ❌ 不必要的複雜步驟 |
`
    },
    {
        ...
        "note": `
這題考察高效能資料庫備份策略，重點是最小化 I/O 影響的備份儲存配置。
...
### 知識點
- **Local SSD**：本機 SSD 高效能儲存
- **MySQL Backup Strategy**：資料庫備份策略
- **I/O Performance Isolation**：I/O 效能隔離

### 選項分析
| 選項 | 分析 |
|------|------|
| A 永久磁碟快照 | ❌ 快照會影響主磁碟效能 |
| **B Local SSD 暫存** | ✅ 高速備份，效能隔離 |
| C gcsfuse 直接備份 | ❌ 網路 I/O 可能影響效能 |
| D RAID10+LVM | ❌ 過度複雜且成本高 |
`
    },
    {
        "topic": "#1",
        ...
        "note": `
這題考察負載測試最佳實踐，重點是全面性擴展性測試的關鍵需求。

### 題目重點
- Compute Engine + Bigtable 服務
- 擴展性壓力測試
- QA 團隊工具導入
- 三項關鍵需求選擇

### 知識點
- **Load Testing Best Practices**：負載測試最佳實踐
- **Environment Isolation**：環境隔離原則
- **Performance Monitoring**：效能監控策略

### 選項分析
| 選項 | 分析 |
|------|------|
| **A Bigtable 效能驗證** | ✅ 核心儲存層效能驗證 |
| **B 獨立測試專案** | ✅ 避免影響正式環境 |
| C 對正式環境壓測 | ❌ 高風險，可能影響服務 |
| D 第三方系統考量 | ✅ 但非最關鍵需求 |
| E 交易重播機制 | ❌ 複雜度高，非必要 |
| **F 詳細監控日誌** | ✅ 效能分析必要條件 |
`
    },
    {
        ...
        "note": `
這題考察組織級 IAM 權限設計，重點是資安團隊的適當可視性權限配置。
...
### 知識點
- **Organization Hierarchy**：組織階層權限
- **Security Team Permissions**：資安團隊權限原則
- **Read-Only Access**：唯讀存取控制

### 選項分析
| 選項 | 分析 |
|------|------|
| A Org viewer + Project owner | ❌ Owner 權限過大 |
| **B Org viewer + Project viewer** | ✅ 完整可視性，安全權限 |
| C Org admin + Project browser | ❌ Admin 權限過大 |
| D Project owner + Network admin | ❌ 缺乏組織層級可視性 |
`
    },
    {
        "topic": "#1",
        ...
        "note": `
這題考察敏捷開發的自動化安全防護，重點是在 CI/CD 流程中整合安全檢查。
...
### 知識點
- **Shift-Left Security**：安全左移策略
- **Automated Security Scanning**：自動化安全掃描
- **CI/CD Security Integration**：CI/CD 安全整合
...
`
    },
    {
        ...
        "note": `
這題考察 GKE 自動擴展配置，重點是對現有叢集啟用 Cluster Autoscaler。
...
### 知識點
- **Cluster Autoscaler**：叢集自動擴展器
- **Node Pool Management**：節點池管理
- **Dynamic Scaling**：動態擴展機制

### 選項分析
| 選項 | 分析 |
|------|------|
| A resize 手動擴展 | ❌ 固定大小非自動擴展 |
| B instances add-tags | ❌ 標籤無法啟用自動擴展 |
| **C update enable-autoscaling** | ✅ 直接啟用叢集自動擴展 |
| D create 新叢集 | ❌ 不必要重建，影響服務 |
`
    },
    {
        "topic": "#1",
        ...
        "note": `
這題考察無伺服器架構選擇，重點是極度變動流量下的最小運維方案。
...
- **Auto-scaling**：自動擴展能力
- **NoOps Management**：零運維管理

### 選項分析
| 選項 | 分析 |
|------|------|
| **A App Engine + Datastore** | ✅ 無伺服器，自動擴展 |
| B Container Engine + PD | ❌ 需要叢集管理 |
| C MIG + Bigtable | ❌ 需要實例組管理 |
| D 單台 VM + Cloud SQL | ❌ 無法應對大流量變動 |
`
    },
    {
        "topic": "#1",
        ...
        "note": `
這題考察雲原生轉型策略，重點是無運維與自動擴展的現代化運算服務選擇。
...
### 知識點
- **Cloud-Native Architecture**：雲原生架構
- **Serverless Computing**：無伺服器運算
...
|------|------|
| A 單純容器化 | ❌ 仍需運維管理 |
| **B GKE + 容器** | ✅ 託管 K8s，自動擴展 |
| **C App Engine Standard** | ✅ 完全無伺服器 |
| D 自訂機型 | ❌ 增加運維複雜度 |
| E 受管群組 | ❌ 仍需部分運維 |
`
    },
    {
        ...
        "note": `
這題考察日誌完整性保護，重點是數位簽章確保日誌真實性與不可否認性。
...
### 知識點
- **Digital Signature**：數位簽章技術
- **Log Integrity**：日誌完整性保護
...
|------|------|
| A 雙重備份 | ❌ 備份非驗證機制 |
| B SQL 權限控制 | ❌ 無法防止授權用戶竄改 |
| **C 數位簽章** | ✅ 密碼學保證真實性 |
| D JSON 格式儲存 | ❌ 格式化非完整性保護 |
`
    },
    {
        "topic": "#1",
        ...
        "note": `
這題考察組織架構設計，重點是資料夾層級權限管理與政策繼承的最佳實踐。

### 題目重點
- Workspace 與 GCP Org 整合
- 開發者專案建立權限
- Production 專案保護
- 集中政策管理

### 知識點
- **Folder-based Organization**：資料夾組織結構
- **IAM Policy Inheritance**：IAM 政策繼承
...
|------|------|
| A 雙組織架構 | ❌ 複雜度高，管理困難 |
| B 僅 Production 資料夾 | ❌ 開發專案權限控制不足 |
| **C Dev+Prod 資料夾** | ✅ 清晰分離，精確權限控制 |
| D 組織外開發 | ❌ 失去集中管理優勢 |
`
    },
    {
        ...
        "note": `
這題考察內容分發網路優化，重點是用 CDN 解決熱門內容存取瓶頸。
...
- **Cloud CDN**：內容分發網路
- **Global Load Balancer**：全球負載均衡
- **Cache Performance**：快取效能優化

### 選項分析
| 選項 | 分析 |
|------|------|
| A gcsfuse 掛載 | ❌ 無法解決熱點問題 |
| B Filestore 快取 | ❌ 手動管理，擴展性差 |
| C CloudSQL 備用 | ❌ 資料庫非多媒體儲存 |
| **D CDN + 負載均衡** | ✅ 全球快取，自動優化 |
`
    },
    {
        "topic": "#1",
        ...
        "note": `
這題考察日誌長期保存策略，重點是 Cloud Logging 與 Cloud Storage 的整合。
...
### 知識點
- **Log Export**：日誌匯出機制
- **Cloud Storage Archive**：雲端儲存歸檔
- **Log Retention Policy**：日誌保存政策

### 選項分析
| 選項 | 分析 |
|------|------|
| **A Filter + Storage 匯出** | ✅ 長期保存標準方案 |
| B Compute Engine 防火牆 | ❌ 與 VPN 日誌無關 |
| C Dashboard 查詢 | ❌ 儀表板非保存機制 |
| D Filter + Pub/Sub | ❌ 訊息佇列非長期儲存 |
`
    },
    {
        "topic": "#1",
...
`
    },
    {
        ...
        "note": `
### 題目重點
Mountkirk Games 分析統計報表遷移到 GCP，需選擇兩個關鍵遷移步驟。

### 知識點
- **Analytics Migration Strategy**：分析系統遷移策略
- **ETL Pipeline Modernization**：ETL 管道現代化
- **BigQuery Schema Design**：BigQuery 資料結構設計
- **Cloud Dataflow Processing**：Cloud Dataflow 處理

### 選項分析
| 選項 | 分析 |
|------|------|
| **A 評估 ETL 到 Dataflow** | ✅ 現代化 ETL 管道核心步驟 |
| **B BigQuery 去正規化計畫** | ✅ 分析型資料庫最佳化 |
| C MySQL 叢集架構圖 | ❌ 不符合雲原生分析需求 |
| D 10TB 到 Cloud SQL 測試 | ❌ SQL 不適合大數據分析 |
| E Cloud Armor SQL 注入 | ❌ 非分析遷移核心議題 |
`
    },
    {
        "topic": "#6",
        ...
        "note": `
### 題目重點
Mountkirk Games 運算工作負載技術架構，需要全球化遊戲服務的高可用性設計。

### 知識點
- **Global Load Balancing**：全球負載平衡
- **Managed Instance Groups**：受管實例群組
- **Gaming Workload Reliability**：遊戲工作負載可靠性
- **Auto Scaling for Games**：遊戲自動擴展

### 選項分析
| 選項 | 分析 |
|------|------|
| A 網路 LB + 可搶佔 VM | ❌ 區域限制且可靠性不足 |
| B 網路 LB + 非搶佔 VM | ❌ 區域限制無法全球服務 |
| C 全球 LB + 可搶佔 VM | ❌ 遊戲服務需要穩定性 |
| **D 全球 LB + 非搶佔 VM** | ✅ 全球服務且高可靠性 |
`
    },
    {
        "topic": "#6",
        ...
        "note": `
### 題目重點
Mountkirk Games 未來導向解決方案設計，利用雲端技術改進，需選擇兩個前瞻步驟。

### 知識點
- **Future-Proof Architecture**：未來驗證架構
- **Container Orchestration**：容器編排
- **Machine Learning Data Strategy**：機器學習資料策略
- **Cloud-Native Development**：雲原生開發

### 選項分析
| 選項 | 分析 |
|------|------|
| **A 儲存資料訓練 ML 模型** | ✅ 未來 AI/ML 預測能力 |
| **B 容器化 + GKE 部署** | ✅ 雲原生彈性架構 |
| C Jenkins + Spinnaker CI/CD | ❌ 傳統工具非雲原生 |
| D 資料結構版本管理 | ❌ 重要但非未來導向 |
| E Linux VM 滾動維護 | ❌ 傳統維運非雲原生 |
`
    },
    {
        "topic": "#6",
        ...
        "note": `
### 題目重點
Mountkirk Games 分析平台對行動網路延遲變化的韌性測試設計。

### 知識點
- **Chaos Engineering**：混亂工程
- **Latency Injection Testing**：延遲注入測試
- **Mobile Network Resilience**：行動網路韌性
- **Failure Testing Strategies**：故障測試策略

### 選項分析
| 選項 | 分析 |
|------|------|
| **A 故障注入軟體注入延遲** | ✅ 專業混亂工程方法 |
| B Compute Engine 模擬客戶端 | ❌ 複雜且非真實網路環境 |
| C 檔案處理前隨機延遲 | ❌ 非網路延遲模擬 |
| D 玩家裝置測試版收集 | ❌ 被動收集非主動測試 |
`
    },
    {
        "topic": "#6",
        ...
        "note": `
### 題目重點
Mountkirk Games 資料庫工作負載技術架構，需要支援不同資料類型的最佳資料庫選擇。

### 知識點
- **Multi-Database Architecture**：多資料庫架構
- **Time Series Data Management**：時間序列資料管理
- **Transactional vs Analytical Workloads**：交易型與分析型工作負載
- **Database Service Selection**：資料庫服務選擇

### 選項分析
| 選項 | 分析 |
|------|------|
| A SQL 時序 + Bigtable 歷史 | ❌ SQL 不適合時間序列 |
| B SQL 替代 + Spanner 歷史 | ❌ 未最佳化各自用途 |
| C Bigtable 替代 + BigQuery | ❌ Bigtable 非交易型 |
| **D 完整多資料庫架構** | ✅ 各資料庫適得其所 |
`
    },
    {
        ...
        "note": `
### 題目重點
Mountkirk Games 遊戲活動時間序列資料庫服務的託管儲存選項。

### 知識點
- **Time Series Database Requirements**：時間序列資料庫需求
- **Cloud Bigtable for Gaming**：遊戲應用的 Cloud Bigtable
- **High-Performance NoSQL**：高效能 NoSQL
- **Real-time Game Analytics**：即時遊戲分析

### 選項分析
| 選項 | 分析 |
|------|------|
| **A Cloud Bigtable** | ✅ 最佳時間序列資料庫 |
| B Cloud Spanner | ❌ 全球一致性過度需求 |
| C BigQuery | ❌ 分析型非即時寫入 |
| D Cloud Datastore | ❌ 文件型非時序最佳化 |
`
    },
    {
        "topic": "#6",
        ...
        "note": `
### 題目重點
Mountkirk Games 遊戲後端平台架構，REST API 通訊，遵循 Google 建議設計。

### 知識點
- **REST API Load Balancing**：REST API 負載平衡
- **Multi-Zone High Availability**：多區域高可用性
- **L7 vs L4 Load Balancer**：七層與四層負載平衡器
- **Game Backend Resilience**：遊戲後端彈性

### 選項分析
| 選項 | 分析 |
|------|------|
| A 多區 + L4 負載平衡 | ❌ L4 不支援 REST API 功能 |
| B 單區 + L4 負載平衡 | ❌ 單區風險且 L4 不適合 |
| **C 多區 + L7 負載平衡** | ✅ 高可用且支援 REST 功能 |
| D 單區 + L7 負載平衡 | ❌ 單區可用性風險 |
`
    },
    {
        "topic": "#7",
        ...
        "note": `
### 題目重點
Mountkirk Games 批次檔案傳輸最佳化，遊戲統計資料上傳到 Cloud Storage，準備 ETL 處理。

### 知識點
- **Batch File Transfer Optimization**：批次檔案傳輸優化
- **gsutil Parallel Operations**：gsutil 並行操作
- **Cloud Storage ETL Pipeline**：雲端儲存 ETL 管道
- **Gaming Data Processing**：遊戲資料處理
...
| A 序列移動 | ❌ 速度慢且移動後原檔案消失 |
| **B 並行複製** | ✅ 最高效能，保留原檔案 |
| C ETL 提取階段 | ❌ gsutil 非 ETL 工具 |
| D ETL 載入階段 | ❌ gsutil 非 ETL 工具 |
`
    },
    {
        "topic": "#7",
        ...
        "note": `
### 題目重點
Mountkirk Games 新遊戲存取舊遊戲 Firestore 資料庫，跨專案權限最小化原則。

### 知識點
- **Cross-Project Service Account**：跨專案服務帳號
- **Firestore Access Control**：Firestore 存取控制
- **Firebase Admin Role**：Firebase 管理員角色
- **Principle of Least Privilege**：最小權限原則

### 選項分析
| 選項 | 分析 |
|------|------|
| A 兩個 SA + 組織管理員 | ❌ 權限過大，管理複雜 |
| B 組織管理員權限 | ❌ 組織級權限過大 |
| **C 單一 SA + 雙專案 Firebase 管理員** | ✅ 權限最小，跨專案存取 |
| D 專案遷移 | ❌ 架構變更成本過高 |
`
    },
    {
        ...
        "note": `
### 題目重點
Mountkirk Games 限制資源僅部署於營運的 Google Cloud 區域，地理位置控制。

### 知識點
- **Organization Policy Constraints**：組織政策約束
- **Resource Location Restriction**：資源位置限制
- **Governance and Compliance**：治理與合規
- **Geographic Resource Control**：地理資源控制
...
`
    },
    {
        ...
        "note": `
### 題目重點
Mountkirk Games 新遊戲網路入口實作，多區域部署，滿足業務技術需求。

### 知識點
- **Global Load Balancer**：全球負載平衡器
- **Multi-Region Game Deployment**：多區域遊戲部署
- **Anthos Ingress Controller**：Anthos 入口控制器
- **Gaming Network Architecture**：遊戲網路架構

### 選項分析
| 選項 | 分析 |
|------|------|
| A Compute Engine 實例群組 | ❌ 非容器化，擴展性較差 |
| B kubemci | ❌ 已棄用的工具 |
| C 基本 GKE + GLB | ❌ 缺乏進階入口功能 |
| **D Anthos Ingress + GKE** | ✅ 企業級多區域解決方案 |
`
    },
    {
        "topic": "#7",
        ...
        "note": `
### 題目重點
Mountkirk Games GKE 遊戲新版本品質評估，建立用戶角度的服務層級指標（SLI）。

### 知識點
- **Service Level Indicators (SLI)**：服務層級指標
- **User-Centric Metrics**：用戶中心指標
- **Game Quality Assessment**：遊戲品質評估
- **SRE Best Practices**：SRE 最佳實務

### 選項分析
| 選項 | 分析 |
|------|------|
| A CPU + 請求延遲 | ❌ CPU 非用戶感知指標 |
| B CPU + 記憶體使用率 | ❌ 基礎設施指標，非用戶體驗 |
| **C 請求延遲 + 錯誤率** | ✅ 直接影響用戶體驗指標 |
| D 伺服器運作時間 + 錯誤率 | ❌ 運作時間非即時用戶感知 |
`
    },
    {
        "topic": "#7",
        ...
        "note": `
### 題目重點
Mountkirk Games 新遊戲平台與 Google Cloud 安全連線，簡化流程，遵循 Google 建議做法。

### 知識點
- **Workload Identity**：工作負載身分識別
- **Kubernetes Security Best Practices**：Kubernetes 安全最佳實務
- **Google Cloud Authentication**：Google Cloud 認證
- **Service Account Management**：服務帳號管理

### 選項分析
| 選項 | 分析 |
|------|------|
| **A Workload Identity + 服務帳號** | ✅ Google 建議的最佳實務 |
| B 基本 Kubernetes Secrets | ❌ 安全性不足 |
| C Secrets + 應用層加密 | ❌ 過度複雜 |
| D HashiCorp Vault | ❌ 非 Google 原生解決方案 |
`
    },
    {
        "topic": "#7",
        ...
        "note": `
### 題目重點
Mountkirk Games 手機遊戲 App 測試，多種 Android/iOS 裝置配置，高效且具成本效益。

### 知識點
- **Firebase Test Lab**：Firebase 測試實驗室
- **Mobile App Testing Strategy**：手機 App 測試策略
- **Cross-Platform Testing**：跨平台測試
- **Cost-Effective Testing**：成本效益測試

### 選項分析
| 選項 | 分析 |
|------|------|
| **A Firebase Test Lab** | ✅ 專業手機測試平台，高效成本效益 |
| B VM 模擬 | ❌ 成本高，非真實裝置 |
| C 容器化測試 | ❌ 無法模擬真實手機環境 |
| D Firebase Hosting | ❌ 非測試平台，功能不符 |
`
    },
    {
        "topic": "#8",
...
### 知識點
- **Cloud Storage**：物件儲存服務特性
- **Cloud Datastore**：NoSQL文件數據庫
- **中繼資料管理**：檔案與使用者關聯

### 選項分析
| 選項 | 分析 |
|------|------|
| **A Storage + Datastore** | ✅ 最佳延遲，分離儲存與索引 |
| B Storage + 自訂中繼資料 | ❌ 查詢能力有限 |
| C 分散式檔案系統 | ❌ 管理複雜度高 |
| D 分散式 + SQL | ❌ 過度複雜，延遲較高 |
`
    },
...
| 選項 | 分析 |
|------|------|
| A Stackdriver Debugger | ❌ 除錯工具非預防測試 |
| **B 單元測試+負載測試** | ✅ 全面覆蓋，生產級驗證 |
| C 雲端端到端測試 | ❌ 已有100%覆蓋率，重複測試 |
| D 金絲雀測試 | ❌ 部署後測試，非預防措施 |
`
    },
    {
...
### 知識點
- **Storage Classes**：Coldline適用於年度存取
- **成本結構**：儲存成本vs存取成本
- **合規性要求**：資料可用性與完整性
...
| 選項 | 分析 |
|------|------|
| **A Coldline Storage** | ✅ 最低成本，適合年度存取 |
| B Nearline Storage | ❌ 成本較高，適合月度存取 |
| C Bigtable | ❌ 高效能資料庫，成本過高 |
| D BigQuery+複雜架構 | ❌ 過度設計，成本最高 |
`
    },
    {
        "topic": "#11",
...
`
    },
    {
        ...
        "answers": ["D"],
        "note": `
這題考察大規模遷移架構設計，重點是10倍成長下的ROI最大化策略。
...
| 選項 | 分析 |
|------|------|
| A App Engine+Datastore | ❌ 需要重大應用改寫 |
| B 部分雲原生 | ❌ Nginx用DM部署非最佳 |
| C Persistent Disk NAS | ❌ 成本較高，擴展性限制 |
| **D 完整託管服務** | ✅ 最佳ROI，最小變更 |
`
    },
    {
        ...
        "answers": ["A"],
        "note": `
這題考察基礎設施自動化部署，重點是Web和交易數據層的標準化部署。
...
- **Cloud Launcher**：預配置解決方案市場
- **託管服務**：Cloud SQL vs 自建MySQL

### 選項分析
| 選項 | 分析 |
|------|------|
| **A DM+Cloud SQL** | ✅ 完整自動化，託管資料庫 |
| B Launcher+DM混合 | ❌ 工具不一致，複雜度較高 |
| C App Engine遷移 | ❌ 需要應用程式重構 |
| D App Engine+Launcher | ❌ 混合平台，不一致 |
`
    },
    {
        "topic": "#12",
        ...
        "answers": ["C"],
        "note": `
這題考察雲遷移中的服務適配性，重點是識別可直接遷移且保持效能優化的服務。
...
### 知識點
- **Cloud Dataproc**：託管Hadoop/Spark服務
- **High Availability**：多區域容錯能力
- **服務託管化**：減少運維負擔

### 選項分析
| 選項 | 分析 |
|------|------|
| A App Engine標準 | ❌ 需要應用程式重構 |
| B 非受管RabbitMQ | ❌ 失去雲端優勢 |
| **C Dataproc HA模式** | ✅ 直接替代，效能優化 |
| D 自訂機器類型 | ❌ 非託管服務，運維負擔重 |
`
    },
    {
        "topic": "#12",
        ...
        "answers": ["D"],
        "note": `
這題考察雲端審計合規性，重點是管理操作的完整可視性與可稽核性。
...
### 知識點
- **Activity Logs**：管理操作記錄
- **Stackdriver Logging**：集中化日誌管理
- **審計追蹤**：合規性證據收集

### 選項分析
| 選項 | 分析 |
|------|------|
| A Stackdriver Trace | ❌ 應用程式追蹤非管理操作 |
| B Monitoring Dashboard | ❌ 監控指標非操作記錄 |
| C IAP存取控制 | ❌ 存取控制非操作記錄 |
| **D Activity+Logging** | ✅ 完整管理操作可視性 |
`
    },
    {
        "topic": "#12",
        ...
        "answers": ["C"],
        "note": `
這題考察Cloud Storage安全最佳實踐，重點是簡化設計下的有效安全控制。
...
| 選項 | 分析 |
|------|------|
| A 自訂角色+CSEK | ❌ 過度複雜，非最簡設計 |
| B 自訂角色+預設加密 | ❌ 自訂角色增加複雜度 |
| **C 預定義角色+預設加密** | ✅ 最佳實踐，最簡設計 |
| D 預定義角色+KMS | ❌ KMS非必要，增加複雜度 |
`
    },
    {
        "topic": "#12",
...
| 選項 | 分析 |
|------|------|
| A 替換為Pub/Sub | ❌ 雲端服務無法本地運行 |
| B MySQL版本降級 | ❌ 限制功能，非最佳做法 |
| C 調整機器規格 | ❌ 表面調整，未解決根本問題 |
| **D 微服務容器化** | ✅ 為雲端做好技術準備 |
`
    }
];
